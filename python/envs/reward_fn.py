...
class MADDPGFormationReward:
    ...
    def __init__(self, safety_threshold=0.5, target_distance=1.5, max_distance=4.0):
        ...
        # ✅ 核心价值观权重分配 (全部补齐了！)
        self.w_link = 4.0      # 连线奖励：连了谁，就跟谁保持 target_distance
        self.w_iso = 5.0    # 孤立惩罚：如果谁也不连，当场重罚
        self.w_sparsity = 0.2   # 稀疏惩罚：每多连一条线扣一点分，逼迫图精简
        
        self.w_separation = 10.0   # 分离度：无差别防互撞的惩罚权重 (底线，给很高)
        self.w_danger = 5.0      # 危险区：撞墙/进入障碍物膨胀层的重罚权重 (保命)
        self.w_jitter = 1.0      # 平滑度：过度抖动/能量损耗的惩罚权重
        self.w_direction = 2.0   # 方向感：保持在老大后方的得分权重
        self.w_geometry = 3.0    # 几何形状：鼓励形成良好的队形奖励权重 (新加的)
    ...
    def compute(self, leader_pos, follower_positions, prev_follower_positions, leader_yaw, in_danger_zone, in_crash_zone, dynamic_connections, corridor_width):
        ...
        for i in range(num_followers):
            ...
            for target_id, weight in targets_with_weights: # 这里完美解包了刚才存入的元组
                ...
                # 加权惩罚：权重越大，误差造成的扣分越疼
                weighted_error_penalty = - (dist_error * weight * self.w_link) 
                
                team_reward += weighted_error_penalty
                reward_details['link_score'] += weighted_error_penalty
        ...
        if any_crash:
            ...
            team_reward -= 20.0 
            reward_details['danger'] -= 20.0
        else:
            team_reward += 2.0 # 存活奖励
