...
class Formation2DEnv(gym.Env):
    def __init__(self, config):
        ...
        self.planning_safe_margin = 1.0  # 设置距离障碍物 1.0 米的安全冗余
    ...
    def _load_or_generate_map(self):
        ...
        map_path = "/home/nankai/formation_test/python/maps/underground_garage.pgm"  # ✅ 改为你的 pgm 文件名
    ...
    def step(self, action):
        ...
        reward, reward_details = self.reward_fn.compute(
            ...
            self.map_grid, self.map_resolution
        )
    ...
    def render(self, mode='human'):
        ...
        # ✅ 4. 绘制编队中的机器人
        # 绘制领航者（红色大圆点）
        self.ax.plot(self.leader_pos[0], self.leader_pos[1], 'ro', markersize=12, label='Leader', zorder=5)
        
        # 为领航者添加前进朝向箭头 (以车头为基准，X-Forward)
        arrow_length = 0.8
        ...
        self.ax.arrow(self.leader_pos[0], self.leader_pos[1], dx, dy,
                      head_width=0.3, head_length=0.4, fc='r', ec='r', alpha=0.8, zorder=5)
        
        # 绘制跟随者（蓝色小圆点）
        for pos in self.follower_pos:
            self.ax.plot(pos[0], pos[1], 'bo', markersize=8, zorder=5)
            
        # ==========================================
        # ✅ 核心修改：根据真实的控制图动态绘制通信连线 (绿色虚线)
        # ==========================================
        # 把所有机器人的坐标放进一个列表，方便通过索引 [i] 和 [j] 访问
        # 索引 0 是领航者，1 到 N 是跟随者
        all_positions = [self.leader_pos] + list(self.follower_pos)
        
        # 检查是否已经有了选定的控制图
        if hasattr(self, 'prev_graph_idx') and self.prev_graph_idx is not None:
            current_graph = self.candidate_graphs[self.prev_graph_idx]
            
            for i in range(self.num_robots):
                for j in range(i + 1, self.num_robots):
                    if current_graph[i, j] > 0:  # 如果图中有连线
                        pos1 = all_positions[i]
                        pos2 = all_positions[j]
                        # 画绿色的虚线代表通信链路
                        self.ax.plot([pos1[0], pos2[0]], [pos1[1], pos2[1]], 
                                     color='green', linestyle='--', alpha=0.8, linewidth=2, zorder=4)

        # ✅ 5. 加上网格和标题
        self.ax.grid(True, linestyle='--', alpha=0.3)
        
        # 在标题里实时显示当前使用的是哪一张图！
        current_graph_text = f"Graph: {self.prev_graph_idx}" if hasattr(self, 'prev_graph_idx') else ""
        self.ax.set_title(f'Formation 2D Env | X-Forward | {current_graph_text} | Step: {self.step_count}')
