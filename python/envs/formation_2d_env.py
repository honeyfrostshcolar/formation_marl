...
class Formation2DMultiAgentEnv(MultiAgentEnv):
    ...
    def _get_single_follower_obs(self, follower_idx):
        ...
        for j in range(self.num_followers):
            if j != follower_idx:
                rel_pos = self.follower_pos[j] - my_pos
        ...
        # 按照距离从近到远排序
        visible_teammates = sorted(visible_teammates, key=lambda x: x['dist'])
        
        # 截取最近的 K 个兄弟
        visible_teammates = visible_teammates[:self.max_visible_teammates]
        
        # 填充观测数组 (Padding)
        teammates_obs = np.zeros((self.max_visible_teammates * 3), dtype=np.float32)
        for i, teammate in enumerate(visible_teammates):
            base_idx = i * 3
            teammates_obs[base_idx] = teammate['rel_pos'][0]     # dx
            teammates_obs[base_idx + 1] = teammate['rel_pos'][1] # dy
            teammates_obs[base_idx + 2] = 1.0                    # is_valid 标志位，代表真实存在
            
        # 剩下的槽位自然保持 0.0，而且 is_valid = 0.0，神经网络看到 0 就会知道这是个假目标
